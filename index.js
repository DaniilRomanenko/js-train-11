...
function clearSet(set) {
  ...
  if(set.size >0){
    set.clear();
    return 'множина очищена';
  }else{
    return 'Множина порожня'
  }
...
function filterAndAdd(mySet, arr) {
  // Створюємо нову множину, куди будемо додавати елементи, які є числами
  ...
  for (const element of mySet) {
    ...
    if (typeof element === 'number') {
      // Якщо елемент є числом, додаємо його до множини numbersSet
      numbersSet.add(element);
    }
  }

  // Очищаємо вихідну множину
  mySet.clear();

  // Перебираємо масив arr
  for (const element of arr) {
    // Додаємо кожний елемент до множини за допомогою методу add
    mySet.add(element);
  }

  // Повертаємо множину, яка містить лише числові значення з mySet
  return numbersSet;
}
...
function sumNumbers(set) {
  ...
  let sum=0;
  // Використовуємо forEach для перебору елементів множини
  set.forEach((element) => {
